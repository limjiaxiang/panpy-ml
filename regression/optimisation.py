...
class GradientDescent:
    ...
    def __init__(self, model, lr=0.01, max_epochs=1000, batch_size=32, random_seed=None):
        self.model = model
    ...
    def _calculate_cost(self, params, x_matrix, y_matrix, scale_x=False, train=False):
        return self.model.cost_function(params, x_matrix, y_matrix, scale_x=scale_x, train=train)

    # initialise parameters to be [-1, 1)
    @staticmethod
    def _init_params(size):
        return (np.random.rand(size, 1).flatten() * 2.0) - 1.0

    def descent(self, x_matrix, y_matrix, print_epoch_info=True):
        curr_params = self._init_params(x_matrix.shape[1])
        ...
        while curr_epoch < self.max_epochs:
            ...
            batch_pred = self.model.predict(x_batch, scale=False, custom_params=curr_params, train=True)
            ...
            if new_params_cost > curr_cost:
                ...
                if new_epoch and print_epoch_info:
                    print(f'Start of Epoch {curr_epoch} Cost: {curr_cost:.2f}, Parameters: {curr_params}')
                    new_epoch = False
        return curr_params
